...
public class FloatLabelEditText extends LinearLayout {
	...
	private int mCurrentApiVersion = android.os.Build.VERSION.SDK_INT,
			 mHintColor, mTextColorFocussed,
			mTextColorUnFocussed, mFloatLabelFocussed, mFloatLabelUnFocussed,
	...
	private void initializeView() {
		...
		mInflater
				.inflate(R.layout.weddingparty_floatlabel_edittext, this, true);
	...
	private void getAttributesFromXmlAndStoreLocally() {
		TypedArray attributesFromXmlLayout = mContext.obtainStyledAttributes(
				mAttrs, R.styleable.FloatLabelEditText);
		if (attributesFromXmlLayout == null) {
			return;
		}

		mHintText = attributesFromXmlLayout
				.getString(R.styleable.FloatLabelEditText_hint);
		mEditText = attributesFromXmlLayout
				.getString(R.styleable.FloatLabelEditText_text);
		mGravity = attributesFromXmlLayout.getInt(
				R.styleable.FloatLabelEditText_gravity, Gravity.LEFT);
		...
		mHintColor = attributesFromXmlLayout.getColor(
				R.styleable.FloatLabelEditText_textColorHint,
				android.R.color.darker_gray);
		mFitScreenWidth = attributesFromXmlLayout.getInt(
				R.styleable.FloatLabelEditText_fitScreenWidth, 0);
		mIsPassword = (attributesFromXmlLayout.getInt(
				R.styleable.FloatLabelEditText_inputType, 0) == 1);
		mTextColorFocussed = attributesFromXmlLayout.getColor(
				R.styleable.FloatLabelEditText_textColorFocussed,
				android.R.color.darker_gray);
		mTextColorUnFocussed = attributesFromXmlLayout.getColor(
				R.styleable.FloatLabelEditText_textColorUnFocussed,
				android.R.color.darker_gray);
		mFloatLabelFocussed = attributesFromXmlLayout.getColor(
				R.styleable.FloatLabelEditText_textColorFloatLabelFocussed,
				android.R.color.darker_gray);
		mFloatLabelUnFocussed = attributesFromXmlLayout.getColor(
				R.styleable.FloatLabelEditText_textColorFloatLabelUnFocussed,
				android.R.color.darker_gray);
	...
	private void setupEditTextView() {
		...
		mEditTextView.setHintTextColor(mHintColor);
		mEditTextView.setText(mEditText);
		mEditTextView.setOnFocusChangeListener(new OnFocusChangeListener() {
			@Override
			public void onFocusChange(View v, boolean hasFocus) {
			    if(hasFocus){
			    	mEditTextView.setTextColor(mTextColorFocussed);
			    }else {
			    	mEditTextView.setTextColor(mTextColorUnFocussed);
			    }
			   }
			});
	...
	private void setupFloatingLabel() {
		...
		mFloatingLabel.setTextColor(mFloatLabelFocussed);
		mFloatingLabel.setTextSize(TypedValue.COMPLEX_UNIT_SP,
				(float) (mTextSizeInSp / 1.3));
	...
	private TextWatcher getTextWatcher() {
		return new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				if (s.length() > 0
						&& mFloatingLabel.getVisibility() == INVISIBLE) {
					showFloatingLabel();
				} else if (s.length() == 0
						&& mFloatingLabel.getVisibility() == VISIBLE) {
					hideFloatingLabel();
				}
			}
		};
	...
	private void hideFloatingLabel() {
		...
		mFloatingLabel.startAnimation(AnimationUtils.loadAnimation(
				getContext(), R.anim.weddingparty_floatlabel_slide_to_bottom));
	}
		...
		return new OnFocusChangeListener() {
			...
			private ValueAnimator getFocusToUnfocusAnimation() {
				if (mFocusToUnfocusAnimation == null) {
					mFocusToUnfocusAnimation = getFocusAnimation(
							mFloatLabelUnFocussed, mFloatLabelFocussed);
				}
			...
			private ValueAnimator getUnfocusToFocusAnimation() {
				if (mUnfocusToFocusAnimation == null) {
					mUnfocusToFocusAnimation = getFocusAnimation(mFloatLabelFocussed,
							mFloatLabelUnFocussed);
				}
		...
		colorAnimation
				.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
					...
					public void onAnimationUpdate(ValueAnimator animator) {
						mFloatingLabel.setTextColor((Integer) animator
								.getAnimatedValue());
					}
				});
	...
	private int getSpecialWidth() {
		float screenWidth = ((WindowManager) mContext
